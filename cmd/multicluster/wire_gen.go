...
func wireApp(confServer *conf.Server, confData *conf.Data, jwt *conf.JWT, logger log.Logger, tracerProvider *trace.TracerProvider) (*kratos.App, func(), error) {
	...
	httpServer := server.NewHTTPServer(confServer, jwt, multiClusterService, logger, tracerProvider)
	grpcServer := server.NewGRPCServer(confServer, jwt, multiClusterService, logger)
